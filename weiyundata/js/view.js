var view = (function(){
    return {
        ...
        createPathNavConstruct: function (datas,id){
                var parents = dataAction.getParentsById(datas,id).reverse();//获取到包括他在内的所有父级元素数据
                
                var str = '';
                var zIndex = parents.length+10;

               for( var i = 0; i < parents.length-1; i++ ){
                   str += '<a href="javascript:;"' +' style="z-index:'+(zIndex--)+'" data-file-id="'+parents[i].id+'">'+parents[i].title+'</a>';
